...
<div style="flex: 1;">
The **tauBayesW** package implements four approaches for Bayesian weighted quantile regression:
...
Also, this vignette compares the performance of **bayesQR** with the three MCMC methods implemented in **tauBayesW** using a simulated dataset and the classic Prostate cancer dataset included in the `bayesQR` package.  
...
- For univariate models, it creates an object of class `bqr_prior`.
- For multivariate models, it creates an object of class `mo_bqr_prior`.

Both objects are **S3 classes** that store the prior information in a structured
way. 
...
Doing an example using tauBayesW

```{r prior-examples}
my_prior <- tauBayesW::prior(
  p = 5,                                    # Number of regression parameters (including intercept)
  type = "MCMC",                            # Prior type: "MCMC" or "EM"
  beta_mean = rep(0, 5),                    # Vector of prior means for regression coefficients
  beta_cov = diag(1000, 5),                 # Prior covariance matrix (large values = vague prior)
  sigma_shape = 0.001,                      # Shape parameter of Inverse-Gamma prior on variance (ALD only)
  sigma_rate = 0.001,                       # Rate parameter of Inverse-Gamma prior on variance (ALD only)
  names = c("Int", "x1", "x2", "x3", "x4")  # Optional coefficient names
)
print(my_prior)
```
...
There are also print() and summary() created for the output objects of tauBayesW

We can expore the return of the object bqr.svy
...
# Extraer solo la tabla de posterior_summary de cada modelo
summaries <- lapply(fits, function(fit) {
  s <- summary(fit)
  s$posterior_summary
})

# Unir y añadir columna Method
summ_df <- do.call(rbind, lapply(seq_along(summaries), function(i) {
  df <- summaries[[i]]
  df$Method <- methods[i]
  df
}))

# Reordenar columnas: Method primero
summ_df <- summ_df[, c("Method", setdiff(colnames(summ_df), "Method"))]
...
```
The `plot()` method in **tauBayesW** makes this straightforward: by passing several quantiles in the `tau` argument, the function can overlay multiple fitted curves in the same plot.
...
We conduct a simulation study to illustrate **tauBayesW** capabilities and compare with **bayesQR** using three different sampling designs: Poisson sampling, stratified sampling, and systematic sampling. Each design simulates complex survey data with known population parameters.
...
# Si no tienes rbern(), creo un alias simple a rbinom:
if (!exists("rbern")) rbern <- function(n, p) rbinom(n, 1, p)

# ---------------------------------------------------------------------
# CONSTRUYE UN DATASET MULTIVARIADO A PARTIR DE TUS FUNCIONES
#   - y1: viene de artificial_data_*
#   - y2: lo generamos con la MISMA X y pesos (misma muestra)
# ---------------------------------------------------------------------
make_mo_dataset <- function(design = c("poi","est","sys"),
                            N = 5000, n = 400, n_est = 4,
                            beta1 = c(2, 1.5),         # usado adentro de tus funciones
                            beta2 = c(-1, 0.8),        # para y2
                            sd2   = 1) {
  design <- match.arg(design)
  # 1) Muestreo con una de tus funciones (produce y1, X y pesos)
  if (design == "poi") {
    s <- artificial_data_poi(N, n)
    X <- s$x_matrix            # columnas: (1, x1)
  ...
  # 2) Construimos y2 con la MISMA X y pesos (misma muestra, otra ecuación)
  #    y2 = X %*% beta2 + ruido
  y2 <- as.numeric(X %*% beta2 + rnorm(nrow(X), sd = sd2))

  # 3) Armamos data.frame para mo.bqr.svy
  df <- data.frame(
    y1 = y1,
    y2 = y2,
    x1 = X[, 2]   # el intercepto lo incluye el modelo automáticamente
  )

  # 4) Matriz de coeficientes verdaderos B (p x d) con p=2 (Int, x1) y d=2 (y1, y2)
  #    Para y1, tus funciones usan beta1 = c(2, 1.5). Para y2 fijamos beta2.
  B <- cbind(beta1, beta2)  # 2x2
...
run_mo_sim <- function(design = "poi",
  ...
  U <- matrix(NA_real_, d, n_dir)
  for (k in 1:n_dir) {
    uk <- rnorm(d); U[, k] <- uk / sqrt(sum(uk^2))
  }

  pr_mo <- mo_prior_default(p = p, beta_cov = diag(1e6, p))
...
out_poi2 <- run_mo_sim(design = "poi", N = 5000, n = 400, n_est = 4,
