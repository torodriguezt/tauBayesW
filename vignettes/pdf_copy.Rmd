...
p <- length(attr(terms(form), "term.labels")) + 1
prior_mean <- rep(0, p)
prior_cov  <- diag(1000, p)   
...
fit_ald <- bqr.svy(
  ...
  burnin    = 10000,
  thin      = 5,
  prior     = list(b0 = prior_mean, B0 = prior_cov),
  print_progress = 5000
)
...
for (q_idx in seq_along(quantiles)) {
  ...
  print(knitr::kable(
    ...
    caption = paste("Poisson Sampling =", tau_val)
  ))
}
```
...
for (q_idx in seq_along(quantiles)) {
  ...
  print(knitr::kable(
    table_result, digits = 4,
    caption = paste("Stratified Sampling =", tau_val)
  ))
}
```
...
for (q_idx in seq_along(quantiles)) {
  ...
  print(knitr::kable(
    table_result, digits = 4,
    caption = paste("Systematic Sampling =", tau_val)
  ))
}
```
