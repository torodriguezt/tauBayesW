...
// --- Solve A^{-1} s using Cholesky: z = A^{-1} s (A SPD) ---
...
// --- Score-likelihood log-posterior (unnormalized in beta) ---
// Prior: beta ~ N(b0, B_inv^{-1})
// s_tau(beta) = X' [ w ⊙ (tau - 1{y - X beta < 0}) ]
// Var[s_tau] ≈ tau(1-tau) * XtWX,  XtWX = X' diag(w) X
// Pseudo-loglik ∝ -0.5 * lambda * s' Var^{-1} s   (determinant const in beta)
...
Rcpp::List _mcmc_bwqr_sl_cpp(const arma::vec& y,
                             const arma::mat& X,
                             const arma::vec& w,   // receives weights already normalized
                             double tau = 0.5,
                             int n_mcmc  = 10000,
                             int burnin  = 2000,
                             int thin    = 10,
