// bayesQR_weight///  * Serie de Maclaurin para z -> 0
//  * Salvaguardas NaN/Inf
//  * sigma^2 actualizado justo tras el M-step
//  * "Jitter" 1e-8 en la diagonal de A -> impide que LDLT se anule
// ------------------------------------------------------------------Salvaguardas NaN/Inf
//  * sigma^2 actualizado justo tras el M-step
//  * "Jitter" 1e-8 en la diagonal de A -> impide que LDLT se anule/  * "Jitter" 1e-8 en la diagonal de A -> impide que LDLT se anule/  * "Jitter" 1e-8 en la diagonal de A -> impide que LDLT se anuleEM_cpp.cpp  - stable version (August 2025)
// ------------------------------------------------------------------
...
// ------------------------------------------------------------------weighted_EM_cpp.cpp  - version estable (agosto 2025)
// ------------------------------------------------------------------
// [[Rcpp::depends(RcppEigen)]]
// [[Rcpp::plugins(cpp17)]]
//
//  * Bessel-K escalado (expon.scaled = 1) -> evita under/overflow
//  * Serie de Maclaurin para z -> 0
//  * Salvaguardas NaN/Inf
//  * sigma^2 actualizado justo tras el M-step
//  • “Jitter” 1e-8 en la diagonal de A → impide que LDLT se anule
// ------------------------------------------------------------------
...
inline double K_ratio(double z) {
  if (z < 1e-8) {                           // Maclaurin up to O(z^4)
    const double z2 = z * z;
...
// ---------------------------------------------------------------------------
//  EM for Bayesian quantile regression with weights
// ---------------------------------------------------------------------------
// [[Rcpp::export]]
Rcpp::List bayesQR_weighted_EM_cpp(
  ...
  double sigma_prev  = (y_u.array() - (x_star * beta_prev).array())
                           .square().mean();
  ...
  for (int iter = 0; iter < max_iter; ++iter) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      const double bs = std::max(w(i) * (2.0 / sigma_prev +
                                 theta * theta * inv_ds), 1e-12);
    ...
    A.diagonal().array() += 1e-8;           // jitter - ensures strict PD
    ...
    const double sigma_curr =
      (term1 + term2 + term3 + b0) /
        ((3.0 * n + a0 + 1.0) / 2.0);
