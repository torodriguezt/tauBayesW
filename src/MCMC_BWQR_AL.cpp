...
/* ------------------------------------------------------------------ *
 * 1.  Inverse-Gaussian sampler IG(μ, λ)                              *
 * ------------------------------------------------------------------ */
                     ...
                     double delta2, double theta)
{
  ...
  for (int i = 0; i < N; ++i) {
    ...
    const double psi    = w[i] * (2.0 + theta * theta / delta2);

    const double mu     = std::sqrt(chi / psi);
    const double lambda = chi;

    v[i] = std::max(rinvgauss(mu, lambda), 1e-8);   // numeric floor
  }
}
 ...
 * ------------------------------------------------------------------ */
// [[Rcpp::export]]
Rcpp::List MCMC_BWQR_AL(const arma::vec& y,
                        const arma::mat& X,
                        const arma::vec& w,
                        double tau           = 0.5,
                        int    n_mcmc        = 50000,
                        ...
                        int    thin          = 10)
{
  ...
  beta_chain.row(0) = arma::solve(X, y).t();          // OLS start
  sigma_chain[0]    = 1.0;
  ...
  const arma::mat B_inv  = arma::eye<arma::mat>(p, p) / 1000.0;  // prior precision
  ...
  for (int k = 1; k < n_mcmc; ++k) {
    ...
    update_v(v, X, w, beta_k, y, delta2, theta);
