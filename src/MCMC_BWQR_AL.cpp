...
/* ------------------------------------------------------------------ *
 * 2.  beta | (sigma, v, ...) - robust to near-singularity                      *
 * ------------------------------------------------------------------ */
                           ...
                           const arma::vec& b_zero)
{
  ...
  ok = arma::chol(L, Sigma + ridge * arma::eye<arma::mat>(Sigma.n_rows, Sigma.n_cols),
                  "lower");
  if (!ok) {                                                // escalate ridge if needed
...
/* ------------------------------------------------------------------ *
 * 4.  v | (beta, sigma, ...)  - GIG(1/2, chi, psi) via IG                           *
 * ------------------------------------------------------------------ */
                     ...
                     double sigma)
{
  ...
  for (int i = 0; i < N; ++i) {
    ...
    const double lambda = std::sqrt(chi * psi);            // correct lambda
